...
void Config::Load() {
    ...
    try {
        ...
        if (j.contains("postProcessing") && j["postProcessing"].is_object()) {
            ...
            ppEnableColorAdjustments   = pp.value("enableColorAdjustments", false);
            ppContrast                 = pp.value("contrast", 0.0f);
            ppSaturation               = pp.value("saturation", 0.0f);
            ppGamma                    = pp.value("gamma", 0.0f);
            ppEnableCrtEffects         = pp.value("enableCrtEffects", false);
...
void Config::Save() {
    ...
    if (hasPostProcessing) {
        ...
        pp["enableColorAdjustments"]   = ppEnableColorAdjustments;
        pp["contrast"]                 = ppContrast;
        pp["saturation"]               = ppSaturation;
        pp["gamma"]                    = ppGamma;
        pp["enableCrtEffects"]         = ppEnableCrtEffects;
